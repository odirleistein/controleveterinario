...
def listar_pessoas(
    busca: str | None = None, tipo_pessoa: str | None = None, db: Session = Depends(get_db),
):
    stmt = select(Pessoa)
    if tipo_pessoa:
...
def atualizar_pessoa(pessoa_id: int, payload: PessoaCreate, db: Session = Depends(get_db)):
    pessoa = db.get(Pessoa, pessoa_id)
    if not pessoa:
        raise HTTPException(status_code=404, detail=NAO_ENCONTRADA)
    _validar_local(db, payload)
...
def desativar_pessoa(pessoa_id: int, db: Session = Depends(get_db)):
